...
# Filter gdf with District
filtered_gdf = gdf[gdf["District"].isin(data['District'])]

filtered_gdf = filtered_gdf.to_crs("EPSG:4326")

# Check if the filtered_gdf is empty
if filtered_gdf.empty:
    print("Filtered GeoDataFrame is empty.")
else:
    # Merge sr_data and plan_data with gdf to get lat/lng based on District
    data = data.merge(filtered_gdf[['District', 'geometry']], left_on='District', right_on='District', how='left')
    ...
    # Add GeoJSON layer of polygons with tooltips showing District
    folium.GeoJson(gdf_json,
                   name='Polygons',
                   style_function=lambda x: {'color': 'blue', 'opacity': 0.5, 'weight': 3},
                   tooltip=folium.GeoJsonTooltip(fields=['District'], aliases=['District:'], labels=True, sticky=True)).add_to(m)
