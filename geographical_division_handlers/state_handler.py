...
# Filter gdf with STATES
filtered_gdf = gdf[gdf["STATE"].isin(data['STATE'])]
...
if filtered_gdf.empty:
    ...
    # Merge sr_data and plan_data with gdf to get lat/lng based on STATE
    data = data.merge(filtered_gdf[['STATE', 'geometry']], left_on='STATE', right_on='STATE', how='left')
    ...
    # Add GeoJSON layer of polygons with tooltips showing STATE
    folium.GeoJson(gdf_json,
                   name='Polygons',
                   style_function=lambda x: {'color': 'blue', 'opacity': 0.5, 'weight': 3},
                   tooltip=folium.GeoJsonTooltip(fields=['STATE'], aliases=['STATE:'], labels=True, sticky=True)).add_to(m)
